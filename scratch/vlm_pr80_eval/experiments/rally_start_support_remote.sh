...
if [[ $# -ne 5 ]]; then
  echo "usage: $0 BACKEND BASE_MANIFEST SUPPORT_MANIFEST ARM OUTPUT_DIR" >&2
  exit 2
fi
...
BACKEND=$1
BASE_MANIFEST=$(realpath -- "$2")
SUPPORT_MANIFEST=$(realpath -- "$3")
ARM=$4
...
readonly BACKEND BASE_MANIFEST SUPPORT_MANIFEST ARM OUTPUT_DIR
readonly EXPERIMENTS_DIR PACKAGE_ROOT VLM_WORK_ROOT
readonly CACHE_ROOT="$VLM_WORK_ROOT/cache/$BACKEND"

for input_path in "$BASE_MANIFEST" "$SUPPORT_MANIFEST"; do
  case "$input_path" in
    "$VLM_WORK_ROOT"/runs/*) ;;
    *) echo "input manifests must be under VLM_WORK_ROOT/runs" >&2; exit 2 ;;
  esac
done
case "$OUTPUT_DIR" in
  "$VLM_WORK_ROOT"/runs/*) ;;
...
finish() {
  ...
  printf '{"backend":"%s","arm":"%s","started_at":"%s","finished_at":"%s","state":"%s","exit_code":%d}\n' \
    "$BACKEND" "$ARM" "$started_at" "$finished_at" "$state" "$status" \
...
timeout --kill-after=30s 25m apptainer exec \
  "${container_args[@]}" \
  "$image" \
  "$python_path" -u -m experiments.rally_start_support_trials run \
  --backend "$BACKEND" \
