...
if [[ $# -lt 3 ]]; then
  echo "usage: $0 BACKEND DETAIL_ROOT OUTPUT_DIR [runner arguments...]" >&2
  exit 2
fi
...
BACKEND=$1
DETAIL_ROOT=$(realpath -- "$2")
OUTPUT_DIR=$(realpath -m -- "$3")
shift 3
EXPERIMENTS_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
PACKAGE_ROOT=$(dirname -- "$EXPERIMENTS_DIR")
VLM_WORK_ROOT=$(realpath -- "$VLM_WORK_ROOT")
readonly BACKEND DETAIL_ROOT OUTPUT_DIR EXPERIMENTS_DIR PACKAGE_ROOT VLM_WORK_ROOT
readonly CACHE_ROOT="$VLM_WORK_ROOT/cache/$BACKEND"

case "$DETAIL_ROOT" in
  "$VLM_WORK_ROOT"/runs/*) ;;
  *) echo "detail root must be under VLM_WORK_ROOT/runs" >&2; exit 2 ;;
esac
case "$OUTPUT_DIR" in
  "$VLM_WORK_ROOT"/runs/*) ;;
  *) echo "output directory must be under VLM_WORK_ROOT/runs" >&2; exit 2 ;;
esac
if [[ -e $OUTPUT_DIR ]]; then
  echo "output directory already exists: $OUTPUT_DIR" >&2
  exit 2
fi
for arm in short_only deterministic broad_facts; do
  if [[ ! -f "$DETAIL_ROOT/inference/$arm/manifest.json" ]]; then
    echo "detail manifest is missing for $arm" >&2
    exit 2
  fi
done
...
apptainer exec \
  "${container_args[@]}" \
  "$image" \
  "$python_path" -u -m experiments.run_detail_trials \
  --backend "$BACKEND" \
  --arm "short_only=$DETAIL_ROOT/inference/short_only/manifest.json" \
  --arm "deterministic=$DETAIL_ROOT/inference/deterministic/manifest.json" \
  --arm "broad_facts=$DETAIL_ROOT/inference/broad_facts/manifest.json" \
  --out "$OUTPUT_DIR" \
  "$@"
