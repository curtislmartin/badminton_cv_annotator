...
case "$MANIFEST" in
  "$VLM_WORK_ROOT"/runs/*) ;;
  *) echo "manifest must be under VLM_WORK_ROOT/runs" >&2; exit 2 ;;
esac
case "$OUTPUT_DIR" in
  "$VLM_WORK_ROOT"/runs/*) ;;
...
if [[ ! -f "$MANIFEST" ]]; then
  echo "replay-pair manifest is missing: $MANIFEST" >&2
  exit 2
fi
...
apptainer exec \
  "${container_args[@]}" \
  "$image" \
  "$python_path" -u -m experiments.replay_pair_trials run \
  --backend "$BACKEND" \
  --manifest "$MANIFEST" \
  --out "$OUTPUT_DIR" \
  "$@"
