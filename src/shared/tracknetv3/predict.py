...
def load_models(tracknet_file, inpaintnet_file=None):
    ...
    Call once, then pass the returned values to predict_video() for each
    clip. This avoids reloading weights per clip (~8s savings each).

    Args:
        tracknet_file (str): Path to TrackNet checkpoint (.pt).
        inpaintnet_file (str, optional): Path to InpaintNet checkpoint.
            None or '' to skip inpainting.

    Returns:
        tuple: (tracknet, inpaintnet, tracknet_seq_len, inpaintnet_seq_len, bg_mode)
            - inpaintnet is None if no weights provided
            - inpaintnet_seq_len is None if no weights provided
    """
    ...
    return tracknet, inpaintnet, tracknet_seq_len, inpaintnet_seq_len, bg_mode


def predict_video(video_file, tracknet, inpaintnet,
    ...
    For batch processing, call load_models() once then call this
    function repeatedly for each video.

    Args:
        video_file (str): Path to input .mp4 file.
        tracknet: Loaded TrackNet model (on GPU).
        inpaintnet: Loaded InpaintNet model (on GPU), or None.
        tracknet_seq_len (int): TrackNet sequence length (from checkpoint).
        inpaintnet_seq_len (int or None): InpaintNet seq length, or None.
        bg_mode (str): Background mode for TrackNet dataset.
        save_dir (str): Directory to write output CSV.
        eval_mode (str): 'weight' (default), 'average', or 'nonoverlap'.
        batch_size (int): Batch size for DataLoader (default 16).
        large_video (bool): Use streaming dataset for large videos (default False).
        max_sample_num (int): Max frames for median image (large_video only).
...
if __name__ == '__main__':
    ...
    predict_video(
        args.video_file, tracknet, inpaintnet, t_seq, i_seq, bg_mode,
        args.save_dir, eval_mode=args.eval_mode, batch_size=args.batch_size,
