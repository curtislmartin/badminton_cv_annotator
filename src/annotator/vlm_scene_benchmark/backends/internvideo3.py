...
class InternVideo3Backend:
    ...
    def __init__(self, *, expected_input_frames: int) -> None:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("InternVideo3 benchmark requires a CUDA device")
        self._torch = torch
        ...
        if devices != {"cuda"}:
            raise RuntimeError(f"InternVideo3 parameters are not wholly on CUDA: {sorted(devices)}")
        dtypes = {str(parameter.dtype).removeprefix("torch.") for parameter in self._model.parameters()}
        if dtypes != {self.cache_dtype}:
            raise RuntimeError(f"InternVideo3 parameter dtypes differ from BF16: {sorted(dtypes)}")
    ...
    def generate(
        ...
        *,
        ...
        require_complete_frame_grid("InternVideo3", frame_indices, self._expected_input_frames)
