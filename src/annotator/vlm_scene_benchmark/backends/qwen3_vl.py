...
def _resolve_model_snapshot(
    ...
    if snapshot.name != SPEC.model_revision:
        raise RuntimeError(
            f"Qwen snapshot resolved to {snapshot.name!r}, expected {SPEC.model_revision!r}"
        )
    return snapshot
    ...
    def generate(
        ...
        *,
        ...
        if len(video_tensor.shape) != 4:
            raise RuntimeError(f"unexpected Qwen video tensor shape {tuple(video_tensor.shape)!r}")
        ...
        spatial_factor = int(video_processor.patch_size) * int(video_processor.merge_size)
        if observed_height % spatial_factor or observed_width % spatial_factor:
            raise RuntimeError(
                f"Qwen video resolution {observed_width}x{observed_height} is not divisible by {spatial_factor}"
            )
        visual_tokens = (
